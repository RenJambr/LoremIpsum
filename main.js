...
//div where text has to show
let textDiv = document.querySelector('.lorem-text');

//the second way to get the same result

// document.querySelector('.btn').addEventListener('click', (e) => {
//     e.preventDefault();
//     textDiv.innerHTML = ""

//     if(isNaN(amount.value) || amount.value < 0 || amount.value > 9 || amount.value == ''){
//         const random = Math.floor(Math.random() * text.length);

//         textDiv.innerHTML += `<p>${text[random]}</p>`

//     } else{

//         for(let i = 0; i < amount.value; i++){
//         textDiv.innerHTML += `<p>${text[i]}</p>`
//         }
...
document.querySelector('.btn').addEventListener('click', e => {
    ...
    //function for random number from 0 to 9
    const random = Math.floor(Math.random() * text.length);

    //if statement to get random paragraph if condition is true
    if(isNaN(value) || value < 0 || value > 9 || value == ""){
        textDiv.innerHTML = `<p class = "result">${text[random]}</p>`
    } else{
        
        //get number of paragraphs which number user input 
        let tempText = text.slice(0, value);

        //maping that number of paragraphs and print that like individual paragraphs
        tempText = tempText
            .map(item => {
